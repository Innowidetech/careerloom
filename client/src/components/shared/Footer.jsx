...
const Footer = () => {
  ...
  const uniqueLocations = Array.from(
    new Set(allJobs.map((job) => normalize(job.location)))
  ).filter(Boolean);
  const uniqueDomains = Array.from(
    new Set(allJobs.map((job) => normalize(job.title)))
  ).filter(Boolean);
  const uniqueCategories = Array.from(
    new Set(allJobs.map((job) => normalize(job.jobType)))
  ).filter(Boolean);
    ...
    <footer className="bg-white text-customBlue py-4 shadow-2xl">
      <div className="container mx-auto px-4">
        <div className="flex flex-col md:flex-row md:justify-between md:items-start">
          {/* Social media links */}
          <div className="flex flex-col items-center mb-6 md:w-1/4 justify-center">
            <img src={logo} alt="logo" className="h-[200px] md:w-[200px]" />
            <h2 className="text-[2rem] sm:text-[2rem] md:text-[2rem] font-bold text-center">
              careerloom
            </h2>
          </div>
                  ...
                  <ul className="space-y-2">
                    {data.map((item) => {
                      const job = allJobs.find(
                        (job) =>
                          normalize(job.location) === item ||
                          normalize(job.title) === item
                      );
                      return (
                        <li
                          key={item}
                      ...
                      {(category === "location"
                        ...
                        return (
                          <li
                            ...
                            className="cursor-pointer hover:text-blue-500"
                            onClick={() =>
                              handleNavigation(category, item, job?._id)
                            }
                          >
                            {item}
                          </li>
