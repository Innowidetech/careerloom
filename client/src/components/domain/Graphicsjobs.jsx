...
function Graphicsjobs() {
  ...
  // Filter jobs for Graphic Design positions
  const filteredJobs = allJobs.filter((job) => {
    ...
    // Check if job title includes any of the specified skills
    const includesSkill = skillsToCheck.some((skill) => jobTitle.includes(skill));
    return includesSkill && matchesLocation && matchesExp;
  });

  // List of skills
  const skills = ["UI/UX Designer", "Motion Graphics Designer", "Adobe Creative Suite Expert"];
    ...
    <>
      ...
      <div className="md:mx-10 min-h-screen">
        <div className="flex overflow-x-auto md:overflow-hidden mb-8 md:mt-16">
          ...
          <img src={SDE3} alt="Dummy Image 3" className="w-full md:w-auto md:h-80 object-cover flex-shrink-0" />
        </div>
        ...
        <div className="flex gap-4 mt-8">
          <div className="p-2 bg-gray-50 md:mt-10 w-full md:w-3/4 flex flex-col gap-4">
            <div className="mb-4">
              <h1 className="text-2xl">Graphics Designer Jobs</h1>
              ...
              <div className="flex gap-2 flex-wrap mt-2">
                {skills.map((skill) => (
                  <button
                    ...
                    className={`px-3 py-1 border rounded-full text-sm hover:bg-orange-100 transition ${
                      selectedSkill === skill ? "border-blue-500 text-blue-500" : "border-orange-300 text-orange-500"
                    }`}
                  >
                    {skill}
                  </button>
                ))}
                <button onClick={() => setSelectedSkill("")} className="px-3 py-1 border border-gray-300 rounded-full text-sm hover:bg-gray-100 transition text-gray-500">
                  Clear Filter
                </button>
              </div>
            </div>
              ...
              filteredJobs.map((job) => (
                <div key={job._id} className="bg-white shadow-md rounded-md p-4 flex flex-col md:flex-row gap-4 items-center">
                  ...
                  <div className="flex gap-4 text-gray-400">
                    <AiOutlineInfoCircle className="hover:text-blue-500 cursor-pointer text-2xl" title="Info" onClick={() => navigate(`/description/${job._id}`)} />
                    <AiOutlineHeart className="hover:text-blue-500 cursor-pointer text-2xl" title="Save" onClick={() => handleAddToWishlist(job)} />
                  </div>
                </div>
              ))
            ) : (
