...
const Slider = () => {
  ...
  const prevSlide = () => {
    setCurrentSlide(
      (prevSlide) => (prevSlide - 1 + uniqueJobs.length) % uniqueJobs.length
    );
  };
  ...
  return (
    <div className="max-w-7xl mx-auto my-10 lg:-ml-10 p-4 md:p-0 md:mx-8 lg:mx-auto">
      <header className="flex justify-between items-center mb-6 md:-translate-x-1">
        <label
          className="text-xl md:text-3xl font-medium mt-4 "
        ...
        <div className="flex">
          ...
          {showArrowsDesktop && (
            <div className="hidden md:flex space-x-2">
              <button
                onClick={prevSlide}
                className="text-black md:p-2 rounded-sm border border-black hover:bg-gray-700"
              >
                <FaArrowLeft className="" />
              </button>
              <button
                onClick={nextSlide}
                className="text-black md:p-2 ml-2 rounded-sm border border-black hover:bg-gray-700"
              >
          ...
          {showArrowsMobile && (
            <div className="md:hidden flex space-x-2">
              <button
                onClick={prevSlide}
                className="text-black p-2 rounded-sm border border-black hover:bg-gray-700"
              >
                <FaArrowLeft />
              </button>
              <button
      ...
      <div className="flex flex-col md:flex-row items-center overflow-hidden">
        {uniqueJobs.map((job, index) => (
          <div
            key={index}
            className={`px-10 md:px-0 relative transition-transform duration-300 md:min-w-[300px] flex-shrink-0 md:mx-2 shadow-md 
                        ${
          ...
          >
            ...
            <h2
              className="text-lg font-medium text-center text-[#4A4E69]"
              style={{ fontFamily: "Inter" }}
            >
              {job.title}
            </h2>
            <div className="flex justify-center items-center space-x-2">
              <Link
                ...
                className="text-orange-600 p-2 hover:underline text-lg font-semibold hover:text-orange-400"
                style={{ fontFamily: "Inter" }}
              >
                Explore
              </Link>
