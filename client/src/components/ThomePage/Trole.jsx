...
const TestRolesSection = () => {
  ...
  const uniqueRoles = Array.from(
    new Set(allJobs.map((job) => job.title.trim().toLowerCase()))
  ).map((normalizedTitle) =>
  ...
  return (
    <>
      ...
      <div className="flex justify-start mx-4 md:mx-8 py-4 max-w-7xl">
        <label
          className="text-xl md:text-3xl font-medium mt-4 lg:mx-12"
      ...
      <div className="flex flex-wrap justify-start gap-4 sm:gap-6 pt-6 pb-8 px-4 sm:px-6 lg:px-10 md:mx-6">
        {uniqueRoles.map((role, index) => (
          <div
            key={role._id}
            className={`flex items-center justify-center min-w-[100px] sm:min-w-[200px] h-[50px] rounded-full text-lg cursor-pointer transition-transform transform hover:scale-105 shadow-md p-3 sm:p-4 ${
          ...
          >
            <p
              className={`text-sm sm:text-base lg:text-lg ${
                hoveredIndex === index ? "text-[#653ea7]" : "text-gray-700"
              } text-center`}
              style={{ fontFamily: "Inter" }}
            >
              {role.title}
